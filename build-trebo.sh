...
# Branding packages on Focal have surprising reverse dependencies. Never let a
# branding cleanup drag GNOME, Ubiquity, GTK, GDM, or other system components
# out with it. Purge a candidate only when apt says that exact package alone
# would be removed.
for pkg in "${purge_candidates[@]}"; do
  mapfile -t would_remove < <(apt-get -s purge "$pkg" 2>/dev/null | awk '$1 == "Remv" {print $2}')
  if (( ${#would_remove[@]} == 1 )) && [[ "${would_remove[0]}" == "$pkg" ]]; then
    ...
    echo "Keeping dependency-sensitive package $pkg; its visible Ubuntu branding will be neutralized instead."
  fi
done

# Remove Ubuntu-specific session choices/extension payloads without touching
# their dependency graph. The generic GNOME session installed above remains.
rm -f \
  /usr/share/xsessions/ubuntu.desktop \
  /usr/share/xsessions/ubuntu-xorg.desktop \
  /usr/share/wayland-sessions/ubuntu.desktop \
...
# Make sure the desktop/installer survived branding cleanup intact.
apt-get -f install -y
apt-get install -y --no-install-recommends \
  gdm3 gnome-shell gnome-session gnome-control-center gnome-terminal nautilus \
  gnome-settings-daemon gnome-tweaks adwaita-icon-theme \
